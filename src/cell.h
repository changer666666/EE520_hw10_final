...
//Cell : can release virus after collision with virus
class CellController : public Process, public AgentInterface {
    ...
    void init() {
        //watch for collision of virus, if collide, will be infected and release other virus
        notice_collisions_with("Virus", [&](Event &e) {
            ...
            if ( ! infected ) {
                counter = 0;
                infected = true; //label to be infected
            }
        });  
        //watch for collision of follow virus, if collide, will be infected and release other virus
        notice_collisions_with("FollowVirus", [&](Event &e) {
            int follow_virus_id = e.value()["id"];
            Agent &follow_virus = find_agent(follow_virus_id);
            virus_ids.push_back(follow_virus_id);
    ...
    void update() {
        ...
        //if infected, change style
        if ( infected ) {
            counter++;
            set_style(infected_style());
            show_baby_viruses();
        }
        //if infected for enough time, cell pops
    ...
    //cell pop function: release many virus to different angle
    void pop() {
    ...
    //show internal virus when infected 
